...
def testPiocher(pioche):
    ...
    if pre-pioche == post-pioche: 
        print("La fonction ne retire pas la carte comme souhaité")
    elif pre-pioche == post-pioche+1: 
        print("La fonction retire correctement la carte")
    else: 
        print("Erreur, le nombre de carte pre-pioche et post-pioche est anormal.")
...
def testExtraireRoyaume(royaume,role):
    ...
    elif nbCarteRoleRoyaume(role) == 0:
...
#Teste la création de la pioche
testPiocheJoueur(JoueurTest1)
#Teste la fonction piocher(pioche)
testPiocherJoueur(pioche(JoueurTest1))
...
testAjouterCarteMain(main(Joueur1))
...
testExtraireFront(front(champBataille(JoueurTest1)),positionExtraire)
